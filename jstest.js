...
function getSecondStarshipName(character) {
  ...
  if (character.starship !== null) {
    return character[1].starship;
  } else {
...
function getStarshipPassengerAndCrewSumTotal(character) {
  // TODO: Add your code here.
  total = character.starship.map(ship => ship.crew + ship.passenger);
  tc = total.reduce((acc, cur) => acc + cur);
  return total;
}
...
function getNthFilm(character, filmNumber) {
  // TODO: Add your code here.
  filmNumber = character[filmNumber - 1];
  if (filmNumber >= 3) {
    return console.log("Sorry can't do that");
  } else {
    return character[filmNumber].films;
  }
}
